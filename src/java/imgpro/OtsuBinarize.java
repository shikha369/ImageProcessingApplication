     ...
     public BufferedImage binarize(BufferedImage original) {
        ...
        for(int i=0; i<original.getWidth(); i++) {
            for(int j=0; j<original.getHeight(); j++) {
                ...
                if(red > threshold) {
                    newPixel = 255; // background in black...so white(255,255,255) pixels are background pixels
                }
                else {
                    newPixel = 0;  // characters written in black...so black(0,0,0) pixels are forground pixels
                }
